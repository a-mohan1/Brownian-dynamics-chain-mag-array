...
c     interactions
c     for a HEXAGONAL array of obstacles
...
c     lspac = lattice spacing (center-to-center distance)
c     xstart = x coord of plane at which obstacle array starts

      subroutine hsevhexarray(x, y, nbeads, a, Robs, lspac, xstart)
      ...
      do 500 ibead = 1, nbeads
         ...
         do 400 m = 1, 2
            ...
            if(x(ibead).le.(x0+a)) then
              cx = x0;
              call hsev1obst(x, y, cx, cy, a, Robs, nbeads, ibead)
            else
